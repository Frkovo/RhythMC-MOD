...
Updated: 2026-09-17 (event-preview op removed; chart speed = blocks/second with BPM-independent time-domain flow; earlier 2026-09-16: sig/glob playback split + arena catalog + BOUNDS_REQ judge-plane view)
...
| 116 | `EVENT_STATE` | `boolean ok`, `String reason`, `int trackId`, `byte channel`, `byte panel`, `int selectIndex`, `int count`, `{double startBeat, double endBeat, double startValue, double endValue, int easing}` × count | Live (v1.7). Whole event list of one channel of the active track, ascending by `startBeat`. `panel=1` is set by `EVENT_REQ LIST` (the mod opens the panel) and then kept in sync with the panel state, so later pushes neither re-open nor close it. `selectIndex` (0-based, `-1` = keep the current selection) tells the panel which segment to select — used by the in-world curve hover `✎ 编辑` button. `ok=false` (play start, session close) makes the mod close the panel; malformed frames are dropped. |
| 117 | `EVENT_HUD` | `int trackId`, `byte channel`, `int count`, `{double startBeat, double endBeat, double startValue, double endValue, byte easing}` × count | Live (v1.7). Current-channel curve for the **mod timeline HUD** (same data as `EVENT_STATE`, no panel flag, `byte` easing). Sent on mod-ready, active-track change, active-channel change, every event mutation, undo/redo and session close — never per tick, so `VIEW_STATE` stays small. Malformed frames are dropped. |
| 119 | `BOUNDS_DATA` | `boolean on`, `double baseX`, `baseY`, `baseZ`, `double planeHalf`, `double boxLength`, `double yOffset`, `int trackCount`, `{int trackId, 10 × (int segCount, {double startBeat, endBeat, startValue, endValue, byte easing} × segCount)}` × trackCount | Live (v1.6). **Static** data for the client-side track outline (answer to `BOUNDS_REQ(118)`): the judge-plane base point (`base*`), the judge-plane half extent (`planeHalf`, default 2.5 = 5x5), the box length along the flight axis (`boxLength`, default 25) and the vertical offset from the base (`yOffset`, default 1.0), then every track's **10 event channels in `EventChannel` order** (`0 Speed, 1 X, 2 Y, 3 Z, 4 SX, 5 SY, 6 SZ, 7 RX, 8 RY, 9 RZ`; `easing` = `Easings` ordinal). Sent when the toggle turns on/off and when chart data changes while it is on — **never per tick**: the mod converts its own audio clock to beats (`TimingManager.calcBeat` semantics) and evaluates the curves locally, so the outline is 60 fps smooth with no per-frame traffic. `on=false` carries `trackCount=0`. |
...
- Audio source is a server-side Dummy file resolved from plugin config (not user upload); the legacy ChartMaker upload path is out of scope here.
...
6. `/charter play [sig|glob] [<from>]` requires `HELLO_ACK.ok=true` and `AUDIO_PUSH_ACK(ok=1)`; it starts an in-memory editor session playback. `sig` (default) plays only the active track at the editor observation point; `glob` plays the overlay set (all tracks by default, `/charter track overlay`) at the separate in-arena play point (`editor.play-arena.*`, default block (0,200,0) of the session world) and pastes the selected arena structure there (`/charter arena use <key>`). `TRANSPORT_PLAY` payload is `double fromMs` + `float speed` (always `1.0`).
...
14. Track events: the mod opens the event panel with `EVENT_REQ(0 LIST, channel)` (the `G` keybind or `/charter events`) and receives `EVENT_STATE(panel=1)`; every edit goes back as `EVENT_REQ(1..4)` and is answered with a fresh `EVENT_STATE` (full list, `panel` unchanged). Closing the panel sends `EVENT_REQ(5 CLOSE)`; starting playback or closing the session sends `ok=false`. Edits are structural: `SPLIT` cuts one segment into two (exactly +1 segment, curve unchanged), `REMOVE` merges a segment into a neighbour (-1 segment, refused for the last one - the panel then offers "reset channel" = `CLEAR`), `UPDATE` moves a shared boundary and/or changes the boundary values (`valueMode` decides whether the next segment's start value follows). Edit-mode notes are rendered from an **absolute projection** that shares no event data with the model: it never applies any event channel, so notes are welded to the ruler grid (distance = beat x grid scale) and neither walking nor editing an event moves them. **Observation point** (server-side rendering anchor, no protocol impact): the paste centre of the per-track arena structure (`origin + editor.arena.offset-*`, one arena per track); playback/audition teleport the player there and the **judgement plane** sits `editor.judge-plane-offset` (default 3.0) blocks in front of it; the edit view keeps its own judgement plane exactly on the cursor's ruler grid line. The runtime flow velocity is `SpeedEvent x playerSpeed` where `SpeedEvent` is a speed in blocks/second (20 = normal) and the accumulated distance integrates `SpeedEvent x (60 / BPM)` over beats, so the flow does not scale with BPM; the edit-mode absolute projection deliberately bypasses that (flat beat-domain distance with the grid scale as `playerSpeed`), which is why the neutral value stays on the ruler grid and negative speed flows backwards. `EVENT_REQ(6 AUDITION)` plays one segment with audio from the segment start to one bar after its end (player teleported to the observation point), stops automatically, returns the player and re-opens the panel; the panel has no play-from-cursor button any more (full-effect listening only). The same channel's curve is mirrored to the mod timeline HUD via `EVENT_HUD(117)` (pushed on mod-ready, track/channel change, every mutation and undo/redo, never per tick); the HUD draws a vertical line where a boundary is discontinuous. Event edits are undoable (`/charter events …` and the panel share the same history; segment numbers are 1-based in chat, 0-based on the wire).
15. Track outline (`B`): the mod sends `BOUNDS_REQ(118, on)`; its local toggle state is driven by the server's `BOUNDS_DATA.on`, so a rejected request never desyncs it. The plugin accepts it only while `glob` playback is running (`STAGE` mode) and answers with `BOUNDS_DATA(119)`: the judge-plane base point, the plane half extent / box length / vertical offset, and **every track's 10 event channels**. It renders nothing itself. Rendering happens entirely in the mod with the vanilla 1.21.11 gizmo API - per track: a `planeHalf*2` square judge plane (semi-transparent red fill + stroke) and a box of that cross-section extending `boxLength` along the flight axis (12 edges with `ignoreOcclusion()` so they show through blocks; side faces are **not** filled) plus a `TRACK #N` world label - all placed from that track's own transform events evaluated at the client's playback beat (identical maths to the note rendering, so outline and notes always agree). The plugin sends a fresh `BOUNDS_DATA` if chart data changes while the toggle is on, and `on=false` when playback stops.
16. Mod playback keys (no new opcodes): `Esc` during playback sends `TRANSPORT_REQ STOP(6)` and is swallowed (no game menu); `Ctrl + Space` sends `TRANSPORT_REQ TOGGLE_PLAY(0)` (play/pause) and is swallowed (no jump) - both only while no screen is open.
...
- Plugin editor (local state, no peer dependency): `editor.default-speed`, `editor.default-length-ms`, `editor.render-distance`, `editor.grid.*` (incl. `bar-blocks` default level and `zoom-levels`), `editor.corridor.*`, `editor.ruler.*`, `editor.placement.*`, `editor.follow.*`, `editor.events.*` (curve sampling / scale mode; segments no longer have a default length), `editor.arena.*` (`schematic`, `offset-x`, `offset-y`, `offset-z`; the per-track arena paste point, whose block centre is the observation point; `published-schematic` = the mocked "published arena" structure used by `/charter arena use village`), `editor.play-arena.x/y/z` (default 0/200/0; `glob` play point: X/Z relative to the session origin, Y absolute), `editor.bounds.*` (`plane-blocks` default 5.0 = the 5x5 judge plane / track XY, `box-length` default 25 = the outline box length along the flight axis, `y-offset` default 1.0 = vertical offset from the judge-plane base; the mod-side `B` outline view) and `editor.judge-plane-offset` (default 3.0; the judgement plane sits that many blocks in front of the observation point).
